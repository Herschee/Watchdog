...
static void tick_handler(struct tm *tick_time, TimeUnits units_changed) {
  ...
  //Check if the watch has been activated and a wakeup_id 
  if (persist_exists(PERSIST_KEY_WAKEUP_ID)) {
    ...
    if(wakeup_query(s_wakeup_id, &wakeup_time))
     ...
     //Convert from epoch time to hours, seconds, and minutes
     int seconds = curr_time % 60;
     int minutes = (curr_time % 3600) / 60;
     int hours = curr_time / 3600;
    
   //Reprints the text layer to the correct time/countdown 
   snprintf(s_uptime_buffer, sizeof(s_uptime_buffer), "   Remaining Time: \n   %dh %dm %ds", hours, minutes, seconds);
...
static void time_window_load(Window *window) {
  ...
  text_layer_set_text(s_time_text_layer, "Remaining Time: ");
